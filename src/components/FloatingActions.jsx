  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-40 flex flex-col gap-2">
      <button
        ...
        className="h-10 w-10 bg-black text-white flex items-center justify-center shadow-lg hover:bg-orange-600 transition-colors relative group rounded-sm"
        title="Copy Email"
        aria-label={copied ? "Email copied" : "Copy email address"}
      >
        {copied ? <CheckCircle2 size={18} /> : <Mail size={18} />}
        <span className="absolute right-full mr-2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-mono pointer-events-none">
